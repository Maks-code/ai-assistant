...
- Локальный веб-интерфейс (`localhost`) с:
  ...
  - формой **Анонимное обращение** с выбором темы и авто-маршрутизацией в блок ЦА;
  - вкладкой **Кабинет блока** (фильтры и смена статусов обращений);
  - вкладкой **Дашборд** (метрики, распределение по блокам/темам/статусам);
  - вкладкой **Админ KB** (отдельный реестр fallback-запросов из чата);
...
- При fallback в чате автоматически создается служебное обращение администратору БЗ (`kb_gap_from_chat`) с дедупликацией повторов по сигнатуре вопроса (окно 24 часа).
...
- Настраиваемый каталог тем/адресатов для анонимных обращений (`app/appeals_catalog.py`).
- Для анонимного обращения обязательно указывается ПУ: автоподстановка из профиля или ручной выбор из справочника.
- Статусы обращений (`Зарегистрировано`, `В работе`, `Нужны уточнения`, `Закрыто`) для управленческого обзора.
...
- `app/appeals_catalog.py` — темы анонимных обращений и маршрутизация `тема -> блок -> email`.
...
В этом журнале также фиксируются служебные чат-обращения по пробелам в базе знаний (`kb_gap_from_chat`, поля `occurrences`, `last_seen_at`).
...
- `GET /api/appeals/topics` — список тем анонимных обращений.
- `POST /api/appeals/anonymous` — регистрация анонимного обращения с авто-маршрутизацией.
- `GET /api/appeals/manager` — список обращений для кабинета начальника блока с фильтрами.
- `POST /api/appeals/{action_id}/status` — смена статуса обращения.
- `GET /api/dashboard/appeals` — дашборд-метрики по обращениям за период.
- `GET /api/admin/fallbacks` — список fallback-обращений из чата для админа БЗ.
- `POST /api/admin/fallbacks/{action_id}/status` — смена статуса fallback-обращения.
...
## Настройка тем и адресатов анонимных обращений
Основная точка настройки: `app/appeals_catalog.py`.

Что менять:
- `APPEAL_BLOCKS` — названия блоков ЦА и email адресатов.
- `APPEAL_TOPICS` — темы, которые выбирает сотрудник.

Дополнительно:
- Справочник доступных ПУ формируется автоматически из `BRANCH_SUBDIVISIONS` в `app/profile.py`.
- В `POST /api/appeals/anonymous` поле `pu_name` обязательно: если в профиле сотрудника уже есть ПУ, оно подставляется автоматически; иначе сотрудник выбирает ПУ вручную из списка.

Логика маршрутизации строится автоматически по связи:
`topic_id -> block_id -> recipient_email`.
